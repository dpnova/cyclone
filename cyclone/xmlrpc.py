...
class XmlrpcRequestHandler(RequestHandler):
    ...
    def post(self):
        ...
        try:
            args, functionPath = xmlrpc.client.loads(self.request.body)
        except Exception as e:
            f = xmlrpc.client.Fault(self.FAILURE,
    ...
    def _getFunction(self, functionPath):
        if functionPath.find(self.separator) != -1:
            ...
            if handler is None:
                raise xmlrpc.client.Fault(self.NOT_FOUND,
        ...
        elif not isinstance(f, collections.Callable):
            raise xmlrpc.client.Fault(self.NOT_FOUND,
    ...
    def _cbRender(self, result):
        ...
        try:
            s = xmlrpc.client.dumps(result,
                methodresponse=True, allow_none=self.allowNone)
        except Exception as e:
            f = xmlrpc.client.Fault(self.FAILURE, "can't serialize output: %s" % e)
    ...
    def _ebRender(self, failure):
        if isinstance(failure.value, xmlrpc.client.Fault):
            ...
            s = xmlrpc.client.Fault(self.FAILURE, "error")

        self.finish(xmlrpc.client.dumps(s, methodresponse=True))
